...
class CustomMuJoCoEnv(gym.Env, abc.ABC):
    ...
    def compile_model_and_data(self):
        ...
        )#<mujoco._structs.MjModel object at 0x7264fe127570>
    ...
    def reset(self, seed: int = None, options=None, **kwargs):
        import os; cjx_debug = os.environ.get('cjx_debug', ''); cjx_debug.lower() == 'true' and print("\033[92m这里ogbench的reset开始===============\033[0m")
        ...
        - Compute the first observation with `compute_observation`.很重要
        """
        super().reset(seed=seed, options=options, **kwargs)#包里的东西
        ...
        self.initialize_episode()#这里会过9次初始化info
        mujoco.mj_forward(self._model, self._data)
        ob = self.compute_observation()#再过一次初始化info<bound method ManipObjEnv.compute_observation of <envs.manip_obj.manip_obj_env.ManipObjEnv object at 0x7265219e60b0>>
        info = self.get_reset_info()#再过一次初始化info
        #import pdb; pdb.set_trace()
        import os; cjx_debug = os.environ.get('cjx_debug', ''); cjx_debug.lower() == 'true' and print("\033[92m这里ogbench的reset结束===============\033[0m")
        return ob, info
    ...
    def step(self, action):
        ...
        ob = self.compute_observation() #<bound method ManipObjEnv.compute_observation of <envs.manip_obj.manip_obj_env.ManipObjEnv object at 0x7d75793d5570>>
        reward = self.compute_reward()
        info = self.get_step_info() #<bound method ManipObjEnv.get_step_info of <envs.manip_obj.manip_obj_env.ManipObjEnv object at 0x7d75793d5570>>
