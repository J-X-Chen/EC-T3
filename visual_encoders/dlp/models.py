...
# modules
from visual_encoders.dlp.modules.modules import KeyPointCNNOriginal, VariationalKeyPointPatchEncoder, CNNDecoder, ObjectDecoderCNN, FCToCNN
from visual_encoders.dlp.modules.modules import ParticleAttributeEncoder, ParticleFeaturesEncoder
# util functions
from visual_encoders.dlp.utils.util_func import reparameterize, create_masks_fast, spatial_transform
from visual_encoders.dlp.utils.loss_functions import ChamferLossKL, calc_kl, calc_reconstruction_loss, VGGDistance, calc_kl_beta_dist
...
class FgDLP(nn.Module):
    ...
    def encode_all(self, x, deterministic=False, noisy=False, warmup=False, kp_init=None, cropped_objects_prev=None,
                   scale_prev=None, refinement_iter=False, var_kp_init=None):
        """
        2-stage encoding:
        ...
        """
        # NOTE: modified from original, added "var_kp_init=None" as input
        ...
        # filter here  # NOTE: modified from original
        confidence_score = particle_stats_dict['logvar'].detach()
        if var_kp_init is not None:
            z_base_var = var_kp_init.detach()
            z_base_var = torch.cat([z_base_var, confidence_score], dim=-1)
        else:
            z_base_var = confidence_score
        # variance filtering
        ...
        # end NOTE: modified from original
    ...
    def forward(self, x, deterministic=False, x_prior=None, warmup=False, noisy=False,
        ...
        # prior proposals
        # kp_p = self.encode_prior(x, x_prior=x_prior, filtering_heuristic='variance')  # original
        kp_p, var_kp = self.encode_prior(x, x_prior=x_prior, filtering_heuristic='none')  # NOTE: modified from original
        ...
        encoder_out = self.encode_all(x, deterministic=deterministic, noisy=noisy, warmup=warmup, kp_init=kp_init,
                                      ...
                                      refinement_iter=refinement_iter, var_kp_init=var_kp)  # NOTE: modified from original
...
class ObjectDLP(nn.Module):
    ...
    def encode_all(self, x, deterministic=False, noisy=False, warmup=False, bg_masks_from_fg=False, train_prior=True,
        ...
        # foreground
        # x: [batch_size, ch, w, h]
        kp_p, var_kp = self.fg_module.encode_prior(x, x_prior=x, filtering_heuristic='none')  # NOTE: modified from original
        kp_init = kp_p if train_prior else kp_p.detach()
