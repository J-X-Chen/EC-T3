...
3. Run: accelerate launch --config_file path/to/accel_conf.yml path/to/train_dlp_accelerate.py
"""
# imports
import os
...
# util functions
from visual_encoders.dlp.utils.util_func import plot_keypoints_on_image_batch, prepare_logdir, save_config, log_line, \
    plot_bb_on_image_batch_from_z_scale_nms, plot_bb_on_image_batch_from_masks_nms, plot_glimpse_obj_on
from visual_encoders.dlp.eval.eval_model import evaluate_validation_elbo
from visual_encoders.dlp.eval.eval_gen_metrics import eval_im_metric
from visual_encoders.dlp.utils.loss_functions import calc_reconstruction_loss, VGGDistance
...
def train_dlp(ds, data_root_dir, tasks=None, batch_size=16, lr=2e-4, device=torch.device("cpu"), kp_activation="tanh",
    ...
    if ds == "hecrl_env":
       ...
       # np_dataset = np.load(os.path.join(data_root_dir, f'ppp-cube-noisy-v0.npz'))
        # dataset = torch.from_numpy(np_dataset['observations'][...].astype(np.uint8, copy=False)).permute(0, 3, 1, 2)
        # dataset = TensorDataset(dataset)
        dataset = HECRL_Env_DS(data_root_dir, mode='train', res=image_size)
        milestones = (20, 40, 80)
    else:
   ...
   # model
    ...
    log_dir = prepare_logdir(runname=run_name, src_dir='visual_encoders/dlp/logs/')
    ...
    if load_model:
        try:
            ...
            unwrapped_model.load_state_dict(torch.load(os.path.join(save_dir, f'{ds}_dlp.pth'), map_location=accelerator.device))
    ...
    for epoch in range(num_epochs):
        ...
        for batch in pbar:
            if ds == "hecrl_env":
                # x = batch[0].squeeze(1).to(device=accelerator.device, dtype=torch.float32) / 255
                x = batch[0].squeeze(1).to(accelerator.device)
                x_prior = x
            elif ds == 'panda_push' or ds == 'libero_spatial' or ds == 'franka_kitchen' or ds == 'ogbench':
                x = batch[0].squeeze(1).to(accelerator.device)
                x_prior = x
            else:
            ...
            optimizer.zero_grad()
            accelerator.backward(all_losses['loss'])
            optimizer.step()
        ...
        if epoch % eval_epoch_freq == 0 or epoch == num_epochs - 1:
            if accelerator.is_main_process:
                ...
                with torch.no_grad():
                    ...
                    batch_indices = torch.arange(mu_tot.shape[0]).view(-1, 1).to(accelerator.device)
                ...
                if dec_objects_original is not None:
                    ...
                    vutils.save_image(torch.cat([x[:max_imgs, -3:], img_with_kp[:max_imgs, -3:].to(accelerator.device),
                                                rec_x[:max_imgs, -3:], img_with_kp_p[:max_imgs, -3:].to(accelerator.device),
            ...
            valid_loss = evaluate_validation_elbo(eval_model, ds, data_root_dir, epoch, tasks=tasks, batch_size=batch_size,
            ...
            if eval_im_metrics and epoch > 0:
                if accelerator.is_main_process:
                    valid_imm_results = eval_im_metric(unwrapped_model, accelerator.device, config,
                                                           val_mode='val',
                                                           eval_dir=log_dir,
...
if __name__ == "__main__":
    config = yaml.safe_load(Path('visual_encoders/dlp/config_dlp.yaml').read_text())

    run_prefix = f"_{config['n_kp_enc']}kp_{config['n_kp_prior']}kpp_{config['learned_feature_dim']}zdim"#_{config['beta_kl']}bkl_{config['kl_balance']}bklb"

    model = train_dlp(ds=config['ds'], tasks=config['tasks'], data_root_dir=config['data_root_dir'], run_prefix=run_prefix,
                      device=None, load_model=config['load_model'],
