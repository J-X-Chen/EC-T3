...
matplotlib.use("Agg")
torch.backends.cudnn.benchmark = os.environ.get("DLP_CUDNN_BENCHMARK", "0") == "1"
torch.backends.cudnn.deterministic = os.environ.get("DLP_CUDNN_DETERMINISTIC", "1") == "1"
...
def train_dlp(ds, data_root_dir, tasks=None, batch_size=16, lr=2e-4, device=torch.device("cpu"), kp_activation="tanh",
    ...
    for epoch in range(num_epochs):
        ...
        for batch in pbar:
            if ds == "hecrl_env":
                # x = batch[0].squeeze(1).to(device=device, dtype=torch.float32) / 255
        ...
        if epoch % eval_epoch_freq == 0 or epoch == num_epochs - 1:
            ...
            valid_loss = evaluate_validation_elbo(model, ds, data_root_dir, epoch, tasks=tasks, batch_size=batch_size,
