...
class HECRLEnvVideo(Dataset):
    def __init__(self, root, mode, ep_len=None, sample_length=20, image_size=128):
        ...
        if len(self.epsisodes) == 0:
            raise RuntimeError(f'No image episodes found under {self.root}')

        if self.EP_LEN is None:
            self.EP_LEN = len(self.epsisodes[0])
...
class HECRLEnvImage(Dataset):
    def __init__(self, root, mode, ep_len=None, sample_length=1, image_size=128):
        if mode == 'val' or mode == 'test':
            mode = 'valid'
        assert mode in ['train', 'valid']
        self.root = os.path.join(root, mode)
